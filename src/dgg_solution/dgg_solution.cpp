...
void get_distance(const string& obj_file_name, const string& DGG_file_name, int source_vert, const string& out_filename)
{
	...
	// for (int i=0;i<rich_model.GetNumOfVerts();i++) {  // all pair shortest path (apsp)
	// 	s_graph->findShortestDistance(i);
	// }
...
int main(int argc, char** argv)
{
	...
	if (argc == 6) {
		...
		if (string(argv[1]) == "SSAD") {
			...
			fprintf(stderr, "Use SSAD (single source) or MSAD (multi source).\n");
			exit(1);
		}
	} else {
		fprintf(stderr, "Wrong argument, usage example:\n'./DGG_Solution SSAD bunny.obj bunny_FD0.0100000000_c5.binary 8 distances.txt'\n");
		exit(1);
	}
