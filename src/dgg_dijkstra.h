...
public:	
	...
	int readDGGFile(const std::string& DGG_file_name) {
		...
		for (int i = 0; i < head_of_DGG.num_of_vertex; ++i) {
			...
			for (int j = 0; j < (int)body_parts.size(); ++j) {
...
public:
	...
	void findShortestDistance(int source) override
	{
		...
		while (!pq.empty()){
			...
			for (int i = 0; i < (int)this->graph_neighbor[u.node_index].size(); ++i){
				int v = this->graph_neighbor[u.node_index][i];
				T w = this->graph_neighbor_dis[u.node_index][i];
	...
	void normalizeDistanceField() {
		...
		for (int i = 0; i < (int)NodeNum(); ++i) {
			...
			if (!std::isfinite(dis[i])) continue;
			max_dis = std::max(max_dis, dis[i]);
		}
		for (int i = 0; i < (int)NodeNum(); ++i) {
