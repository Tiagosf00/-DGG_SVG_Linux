	...
	double const_for_theta, int thread_num)
{
	...
	// long long thread_degree = 0;
	// ichPropogateHead(heads[0], graph_part_file_names[0], accuracy_control_parameter, theta, model, 1, 0, 0, thread_degree);
	...
	for (int i = 0; i < thread_num; ++i) {
		tt[i] = std::thread(
			&ichPropogateHead, std::ref(heads[i]), std::ref(graph_part_file_names[i]), std::ref(accuracy_control_parameter),
			std::ref(theta), std::ref(model), thread_num - i, 0, 0, std::ref(thread_degree[i])
		);
	}
	for (int i = 0; i < thread_num; ++i) {
		tt[i].join();
...
void SVG_ICH_Precompute_Multithread(const std::string& input_obj_name, int fixed_K, int thread_num)
{
	...
	// long long thread_degree = 0;
	// ichPropogateHead(heads[0], graph_part_file_names[0], 0, 0, model, 1, 1, fixed_K, thread_degree);
